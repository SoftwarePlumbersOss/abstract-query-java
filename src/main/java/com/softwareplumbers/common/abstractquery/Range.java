	...
	static Range fromOpenRangeOrValue(JsonValue obj, Function<Value,Range> operator) {
		...
		if (obj instanceof JsonObject) {
			...
			if (propname.isPresent()) {
				JsonValue value = asObj.get(propname);
...
private static class Unbounded extends Range {
	...
	public boolean equals(Object range)	{ return range instanceof Unbounded; }
...
private static abstract class OpenRange extends Range {
	...
	public boolean equals(Object range)	{ 
		if (range instanceof OpenRange)
			return this.operator.equals(((OpenRange)range).operator) 
					&& this.value.equals(((OpenRange)range).value); 
		else
			return false;
	}
	...
	public boolean equals(Between range) { 
		return this.lower_bound.equals(range.lower_bound) 
			&& this.upper_bound.equals(range.upper_bound); 
	}
	
	public boolean equals(Object range) {
		return range instanceof Between && equals((Between)range);
	}
...
*/
	...
	public boolean equals(Equals range)						{ 
		return Boolean.TRUE.equals(ValueComparator.getInstance().equals(this.value, range.value));
	}
	
	public boolean equals(Object other) {
		return other instanceof Equals && equals((Equals)other);
	}
	...
	public Range intersect(Range range) {
		...
		if (range instanceof LessThan || range instanceof LessThanOrEqual) {			
			...
			if (value.type == Value.Type.PARAM || rvalue.type == Value.Type.PARAM) {
				// tricky - a < z && a <= z -> a < z
				if (value.type == rvalue.type && comparator.equals(value, rvalue)) return this;
		...
		if (range instanceof GreaterThan || range instanceof GreaterThanOrEqual) {
			...
			if (value.type == Value.Type.PARAM || rvalue.type == Value.Type.PARAM) {
				...
				if (value.type == rvalue.type && comparator.equals(value, rvalue)) return null;
	...
	public Range intersect(Range range) {
		...
		if (range instanceof GreaterThanOrEqual) {
			...
			if (value.type == Value.Type.PARAM || rvalue.type == Value.Type.PARAM) {
				...
				if (comparator.equals(rvalue, this.value)) {
	...
	public Range intersect(Range range) {
		...
		if (range instanceof LessThanOrEqual) {
			...
			if (value.type == Value.Type.PARAM || rvalue.type == Value.Type.PARAM) {
				...
				if (value.type == Value.Type.PARAM && this.value.equals(rvalue)) return new Equals(this.value);
				return new Intersection(this,range);
			} else {
...
//				console.log('2',known_bounds, this.known_bounds, range);
	...
	*/
	...
	public boolean equals(Intersection range) { 
	...
	public Range bind(Map<String,Value> parameters) {
		...
		for (int i = 0; i < parameters.size() && result != null; i++)
			result = result.intersect(parametrized_bounds.get(parameters.get(i)).bind(parameters));
		return result;
	}
}
}
